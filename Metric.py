...
class DatabaseManager:
    ...
    def connect(self):
        ...
        try:
            ...
            st.write("Kết nối đến database thành công.")
...
class Forecasting:
    ...
    def get_API(self, key):
        querystring = {"keyword": f"{key}", "country": "vn"}
        headers = {
    ...
    def forecast_weather(self, df, forecast_days, time_step=30):
        ...
        plt.title(f'Dự báo nhiệt độ trung bình trong {forecast_days} ngày tới')
        plt.legend()
        plt.show()
    ...
    with st.expander("VIEW ORDER DATASET"):
        showData = st.multiselect(
            ...
            options=[
                ...
                "product_id", "brand", "type", "price", "itemLocation", 
                "weather", "address", "sex", "age", "lat", "lon"
            ],
            default=[
                "order_time", "orderdetail_id", "order_id", "customer_id", 
                "product_id", "brand", "type", "price", "itemLocation", 
...
#toado = pd.read_csv("toa_do_data.csv")
...
def sideBar():
    ...
    if selected == "Forecasting":
        ...
        api_key = "5f47f5e72fmshef2c83d5fc337cbp14dd4ajsnfe6293282368"
        ...
        if key_forecasting:
            result_df= forecasting.get_API(url)#brand_forecasting,type_perfume_forecasting)
        else:
            None
        try:
        ...
        with col1:
            ...
            forecastlist, plt_obj_weather = forecasting.forecast_weather(df,forecast_days, time_step=30)
        ...
        with col2:
            try:
                ...
                None
            st.write(f"Kết quả dự đoán thời tiết trong {time_weather_forecasting} ở {location_forecasting}")
